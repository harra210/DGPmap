...
echo ""
#
## Now all swarmfiles are hopefully generated successfully but as a final sanity check output the first line of each swarmfile to std out for the user to verify the syntax of the swarmfile appears correct.
#
cd "$homedir"
#
...
echo ""
#
## Functions to define whether to submit fully to the cluster or partially submit files to the cluster.
#
pipelinesubmit() {
...
#
jobid8=$(swarm -f hc_gathergvcfs.swarm -g 3 -t 4 -b 2 --gres=lscratch:75 --time 30:00 --partition=quick --module GATK/4.4.0.0,samtools --logdir ~/job_outputs/gatk/gathervcfs/"$SWARM_NAME"_GatherHC --sbatch "--mail-type=ALL,TIME_LIMIT_90 --dependency=afterok:"$jobid7" --job-name "$SWARM_NAME"_GatherHC")
...
while true; do
	case "$promptA" in
		[YyEeSs] ) pipelinesubmit; break;;
		[NnOo] ) echo "See troubleshooting scripts"; exit 1;;
		* ) echo "Please answer yes or no."; return;;
	esac
done
## TO DO: Determine if this is proper. Run Shellcheck.
