...
cd $FQ_DIR
#
## For the Illumina-style fastQ inputs
if [ "$FQ_IN" = "Illumina" ];
	then
...
#
	echo "cd "$MD_DIR"; cp "${FN[0]}".sort.md.bam /lscratch/\$SLURM_JOB_ID; cp "${FN[0]}".sort.md.bam.bai /lscratch/\$SLURM_JOB_ID && gatk --java-options \"-Xmx4G\" ApplyBQSR -R "$CanFam4_Ref" --tmp-dir /lscratch/\$SLURM_JOB_ID -I /lscratch/\$SLURM_JOB_ID/"${FN[0]}".sort.md.bam -XL "$interval_list" -O "$BQSRchr_DIR"/"${FN[0]}".chrY.BQSR.bam -bqsr-recal-file "$TBL_DIR"/"${FN[0]}".bqsrgathered.reports.list --preserve-qscores-less-than 6 --static-quantized-quals 10 --static-quantized-quals 20 --static-quantized-quals 30" >> "$homedir"/bqsr_ApplyBQSR.swarm
#
...
# Generate a while loop for the intervals to create the per chromosome BAM files (possible to use CRAM here?)
	...
	exit 1 # If there is an error in this main loop then the script exits out and quits.
fi
